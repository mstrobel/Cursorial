...
{
    public static readonly StyleDeltaTemplate Identity = default;
    ...
    /// <summary>Copy this template while adjusting only the <see cref="Foreground"/>.</summary>
    public StyleDeltaTemplate WithForeground(IBrush? foreground) => this with { Foreground = foreground };

    /// <summary>Copy this template while adjusting only the <see cref="Background"/>.</summary>
    public StyleDeltaTemplate WithBackground(IBrush? background) => this with { Background = background };

    /// <summary>Copy this template while adjusting only the <see cref="UnderlineColor"/>.</summary>
    public StyleDeltaTemplate WithUnderlineColor(IBrush? underlineColor) => this with { UnderlineColor = underlineColor };

    /// <summary>Copy this template while adjusting only the <see cref="UnderlineShape"/>.</summary>
    public StyleDeltaTemplate WithUnderlineShape(UnderlineStyle? shape) => this with { UnderlineShape = shape };

    /// <summary>Copy this template while adjusting only the <see cref="Mode"/>.</summary>
    public StyleDeltaTemplate WithMode(IBlendingMode? mode) => this with { Mode = mode };

    /// <summary>Copy this template while adjusting only the <see cref="Hyperlink"/>.</summary>
    public StyleDeltaTemplate WithHyperlink(Hyperlink? hyperlink) => this with { Hyperlink = hyperlink };

    /// <summary>Force <paramref name="flags"/> ON in addition to whatever this template already does.</summary>
    public StyleDeltaTemplate Applying(TextAttributes flags) => Composed(PartialStyle.WithApplied(flags));

    /// <summary>Force <paramref name="flags"/> UNSET (no opinion) in addition to whatever this template already does.</summary>
    public StyleDeltaTemplate Ignoring(TextAttributes flags) => WithFlags(Resolved.Ignoring(flags));

    /// <summary>Force <paramref name="flags"/> to keep their current status while ignoring all other booleans.</summary>
    public StyleDeltaTemplate Keeping(TextAttributes flags) => WithFlags(Resolved.Ignoring(PartialStyle.Booleans & ~PartialStyle.Require(flags)));

    /// <summary>Force <paramref name="flags"/> to IGNORED (no opinion) with all others booleans keeping their current status.</summary>
    public StyleDeltaTemplate Dropping(TextAttributes flags) => WithFlags(Resolved.Ignoring(PartialStyle.Booleans & ~PartialStyle.Require(flags)));

    /// <summary>Force <paramref name="flags"/> OFF in addition to whatever this template already does.</summary>
    public StyleDeltaTemplate Removing(TextAttributes flags) => Composed(PartialStyle.WithRemoved(flags));
    ...
    public StyleDeltaTemplate Underlining(UnderlineStyle? shape = null, IBrush? color = null)
    ...
    /// </remarks>
    public StyleDeltaTemplate Composed(in PartialStyle op)
    {
        var folded = Resolved.Then(op);
        return this with { Clear = folded.Clear, Xor = folded.Xor };
    }

    private StyleDeltaTemplate WithFlags(in PartialStyle op)
    {
    ...
    /// <c>b</c>, <c>s</c>.
    /// </remarks>
    public StyleDeltaTemplate Then(in StyleDeltaTemplate next)
    {
        return new StyleDeltaTemplate
               {
                   ...
                   // ...and the fallback is only THIS template's shape when this one did not itself remove
        ...
        bool RemovesUnderline(in StyleDeltaTemplate t) => t.RemovedAttributes.HasFlag(TextAttributes.Underline);
    }
    ...
    /// <returns>
    /// A partial style derived from this template with its brushes resolved at the specified
