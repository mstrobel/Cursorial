...
`Interpolator.For<T>()` / `Register<T>()` — pre-seeded: double, int, `Color` (`Cursorial.Animation`); `PointD`, `Size`, `Rect`, `RelativePoint`, `IBrush`, `CompositeParameters`, `Margins` (`Cursorial.Drawing`); throws with a "register or specify" message for unknown `T`. Threading pinned: process-global; registration at startup on the UI thread (DEBUG-asserted); lock-free immutable-snapshot reads (keeps the multi-session door open). `MarginsInterpolator` lands additively in `Cursorial.Drawing` beside the `Size`/`Rect` family: per-side linear, rounded, **signed** (amended P2.6 — S1's margins are signed per matrix LD19, so tracks may legitimately interpolate through negative values; no interpolator exists yet, so this is a P8 authoring note, not a code change). Decided: `Opacity` is `double` 0–1 with coerce-clamp (WPF-familiar, easing-friendly; byte quantization happens in S1's composite mapping, where record equality makes the compositor the final no-op gate).
...
2. **Control themes:** one selector-less `Style` per `Type` key, `Children` rooted at `^` (incl. `^:access-keys` underscore rules and `^.obscured` modal dimming via `Theme.ObscuredOverlayBrush`), with a `Template` setter holding a `ControlTemplate` (whose `Content` is the `ITemplateContent`). **Every color-bearing setter is a `ResourceReference` into the palette** — the inheritance spine: overriding one `ThemeKeys` entry at a nearer scope re-skins every control with zero template work. **Implementation status (P5 known gap):** the palette-spine wiring (`ResourceReference` color setters → re-skin-via-one-key) is the **R2** work item and is **not yet wired** — the P5 built-in control themes (`ControlThemes`) author their pens/brushes as constants, so overriding a `ThemeKeys` color entry does not re-skin the built-in controls today. The `GlyphSet` keys (`CheckBoxGlyphs`/`RadioGlyphs`/`ScrollArrowGlyphs`) *are* live resource reads; the color keys become live when R2 lands (matrix rows C99/C100/C102 assert the current P5 reality and flip to the full contract at R2). The `ThemeKeys` XML docs carry the same caveat.
