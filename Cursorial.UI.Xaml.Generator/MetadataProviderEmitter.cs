...
internal sealed class MetadataProviderEmitter
{
    ...
    /// <summary>
    /// Emits the provider source for <paramref name="types"/> (resolved closed set), or null if empty. When
    /// <paramref name="installModuleInit"/> is false the provider is emitted but NOT auto-installed (no
        ...
        IReadOnlyList<(string Path, string Expr)>? statics = null)
    {
        ...
        if (installModuleInit)
        {
            ...
            // so an APP consuming assembly uses it automatically without any explicit opt-in. Suppressed for a
            // library (CursorialXamlInstallProvider=false) so it doesn't hijack the app's default.
