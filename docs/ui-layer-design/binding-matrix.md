...
- **BD13 — anchors are mutually exclusive, validated at `CreateExpression`.** Setting more than one of `Source` / `ElementName` / `RelativeSource` throws `InvalidOperationException` naming the conflict. A default-source (no anchor) binding on a non-`UIElement` target is an install-time error (`SourceMissing` + trace) — use `Source`. PIN (doc §6.4; spec §2.1).
...
| B44 | a non-`UIElement` `UIObject` target; default-source `new Binding("Name")` | `Install` | install-time error: `SourceMissing` + a trace naming "non-UIElement target has no DataContext; use Source" | PIN (BD13) |
...
## 7. Modes, triggers, and target → source write-back — B71–B92 *(B0; the LostFocus-routed leg is B1)*
...
| B162b | continue B162a | `vm.IsDirty = true` | the watch delivers `true` ⇒ the condition is met ⇒ the rule **activates synchronously** (no frame pump); `a.P == 5`; one notify | PIN (doc §3.3/§6.8 — live When flip) |
...
| B187 | under full-lowering, an `x:DataType`-scoped `{Binding}` that the compiled lane DECLINES (multi-hop/indexer/method path, explicit Source/ElementName/RelativeSource, Converter/StringFormat/FallbackValue, unrecognized Mode, or a static/not-found/not-readable leaf) but the reflective lane still emits a working binding for | generator analysis (B3) | a **CURG2002 Info** diagnostic at the binding's `.xaml` position names WHY it stayed reflective (the binding works — Info, not Warning); it does NOT fire for a successfully-compiled single-hop binding, for a whole-DataContext (`.`) binding, for a non-`x:DataType` binding, or for a Converter binding the reflective lane also drops (that is the CURG3001 gap). **REALIZED (X5/B3 — P1A):** `TryEmitCompiledBinding` threads the bail reason; `EmitReflectiveBinding` reports whether it emitted; `EmitBinding` raises `Context.Info` → `CURG2002`. Test: `LoweringGeneratorTests.LoweringOptIn_ReflectiveFallback_EmitsCurg2002Info`. | PIN (doc §6.13 B3) |
