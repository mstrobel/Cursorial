...
**Built after v1 (2026-09-13, Shift+Esc):** Shift+Esc is the exit from any depth in one stroke — the Alt pre-stage routes it to `IKeyTipController.PopAllLevels` (every drilled surface closes top down, exactly as repeated Esc would) and then through the same cue-off that Esc-at-root takes, so the overlay, the cue and the focus snapshot resolve as one exit; at the root it is simply Esc. Plain Esc keeps backing out one level at a time.
