...
- `BindingPriority` (high→low): **Animation > LocalValue > Style (single slot; within-slot ordering = styling engine's `StyleSortKey` via frames) > Default**, with template-local provenance and inheritance integrated per store design. Bindings are producers at a priority (`BindingEntry<T>`), animations via `AnimatedValueHandle<T>`.
