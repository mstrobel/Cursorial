...
1. Ladder (strong→weak), **amended 2026-07-12 (the completed Avalonia lattice — the activator split, PD26)**: `Animation(-100) > LocalValue(0) > StyleTrigger(50, the CONDITIONAL style slot — rules carrying any pseudo-class/.class/When condition, within-slot StyleSortKey) > Template(75, the template-instantiation lane) > Style(100, the RESTING style slot — purely structural rules, within-slot StyleSortKey) > Inherited(200, resolution-only) > Default(300, resolution-only)`. `Unset = int.MaxValue` is an internal sentinel, never reported by `GetValueSource` — nor, **as widened 2026-08-09**, carried on any lane-bearing notification: the four ordinary channels of rule 5, whose lane is `UIPropertyChangedEventArgs.Priority` on the `Changed`/virtual pair and the `priority` argument on the two observer interfaces. (The A20 winning-base channel is not an exception, it simply has no lane — `OnBaseValueChanged` carries `isAnimated` and no `BindingPriority` at all.) A write whose replaced lane resolves to nothing reports the storeless tier underneath it instead, per rule 2 and A11 (M118/M127 at rest; M307/M307a for the PD18 re-entrant window — `SetCurrentValue` reading `BasePriority` from inside `Reevaluate`'s retraction). The second such window, `CoerceValue` reading `EffectivePriority` after its own base dispatch, is closed differently: the notification that leaked the sentinel there was a *republication of a contribution superseded underneath the suspended operation*, and it no longer happens at all (the M233a–M233d seam family, 2026-08-09) — **an operation must publish only a change that is still its own and still a change** — so the substitution is not reached rather than being made. That leaves the arm live only for `SetCurrentValue` (M307/M307a); in `CoerceValue` it is a documented-dead backstop. The **Template lane** (§20, added 2026-06-16, PD24 as amended) carries everything a **control** template *authors* on its parts — a literal `SetValue`, a `{TemplateBinding}`/`{Binding}`, a `SetResourceReference`: BELOW `StyleTrigger` so state-driven looks pierce a template's authored part values while active, ABOVE resting `Style` so a template author's literals and TemplateBinding plumbing are the part's resting truth (a broad structural rule cannot wreck template wiring; re-skinning at rest flows through the control's own properties via the forwarding spine, or through conditional rules). *History:* the 2026-06-16 half-adoption put ALL styles above Template (the inverse of WPF, motivated by the close-button repro); it made template literals useless in the other direction and was completed into the Avalonia lattice on 2026-07-12 — the §20 rows carry both pins in their history. The lane is *not* a producer `SetValue`/`Bind`/a frame accept directly: an ambient template-instantiation scope (open while a control template's content tree is built) reroutes the ordinary local-lane producers to Template; outside the scope they stay LocalValue (PD24 — `DataTemplate.Build` deliberately does NOT open it).
...
## 15. Coercion and validation (M230–M243, plus the M233a–M233d seam family)
...
| **The M233a–M233d seam family** *(added 2026-08-09; re-cut the same day — the four rows below were originally two over a byte-identical fixture)* | — | — | `RecoerceLocal` dispatches its A20 winning-base change (user code, PD18) BEFORE it writes the effective lane, so a base observer can recompute the entry underneath the suspended operation. The pin is one rule — **an operation must publish only a change that is still ITS OWN and still a change** — enforced by two tests on resume: the stored **base** must still be the one this operation wrote (superseded?) and the stored **effective** must still differ from it (anything left to say?). Neither is `HasLocal`: every store site that assigns the effective lane on an un-animated entry assigns `Value = BaseValue` in the same breath (`SetLocalValue`, `SetCurrentValue`'s M121 in-place overwrite, `Reevaluate`, `EndAnimation`) — this method is the only one that holds them apart — so a completed nested operation always trips one of the two, and a `!HasLocal` disjunct would be **unkillable dead code**. Each row below kills a distinct mutant | — |
| M233a *(the `default(T)` collision)* | an INHERITING `PcDyn`-shaped property; `root.L(42)`; on **mid** `L(250)` with ceiling 100 (eff=100), then ceiling **lowered to 0**; a winning-base observer (A20) on mid that issues exactly one `CV` from its first delivery, plus a plain subscription of the same observer; an `InheritedProbe` on the leaf | `mid.CoerceValue` | `coerced` = `min(250,0)` = **0** — exactly the `default(int)` `Reevaluate` stores into `BaseValue` when it retracts the last contribution, so the base test AGREES on resume and only the no-change test stops the operation. **Kills the no-change-test mutant.** Expected: exactly ONE ordinary delivery on mid, `(100→42, Inherited)` — the nested `CV`'s promotion, at the storeless tier the read now answers (**rule 2 / PD10 — `Reevaluate`'s Unset-promotion arm, the same substitution M20 makes; NOT A11**, which is `SetCurrentValue` reporting the REPLACED lane, and nothing here grafts) — and exactly ONE propagated delivery on the leaf, `(100→42, Inherited)`, with the ordinary virtual silent (PD22). Without the no-change test the operation also announces `(0→0, Inherited)` and **fans that 0 out over the subtree** that was just truthfully told 42. A20 sees `(100→0)` then `(0→42)`; eff=42 and `IsSet`=false on both nodes. *Superseded:* the original row pinned a second ordinary delivery `(0→250, Inherited)` and asserted only its lane; both values were engine residue (`0` is `Reevaluate`'s reset, `250` the resurrected contribution) — M233b is why. Its replacement asserted the same list as M233b over the same fixture, which pinned nothing M233b did not | PIN (PD18 × PD10; M233 unchanged at rest) |
| M233b *(the resurrection)* | as M233a but on the **leaf**, ceiling **raised to 300** (so `coerced`=250 ≠ `default(int)`) | `leaf.CoerceValue`, then read | The re-entrant `CV` retracts the local contribution while the operation is suspended — `Reevaluate` resets `BaseValue` to `default(int)`, notifies `(100→42, Inherited)` and routes reads to the storeless tier — and the operation resumed and stored `coerced` anyway, **resurrecting** it. **Kills the base-test mutant** (250 ≠ 0). Expected: the retraction is the ONLY ordinary delivery; eff=**42**, `IsSet`=false, `GetBaseValue`=42, src=`Inherited`; A20 sees `(100→250)` then `(250→42)`. The read answered 42 even before the fix — the damage was an *announcement the read disagreed with* (`new`=250) plus the stale `Value` residue behind it, so the delivery list is the discriminating assertion and the reads are the agreement contract | PIN (PD18 × A9/A20; M232/M233 unchanged at rest) |
| M233c *(the replacement)* | as M233b, but the observer issues one re-entrant `L(7)` (a DIFFERENT value) instead of `CV` | `leaf.CoerceValue` | The retraction's twin: `HasLocal` never goes false, so only the stored base witnesses the supersession. Within the lane last writer wins (§2.2) and the nested write already published. **Kills the base-test mutant** from the replacement side. Expected: eff=**7**, `GetBaseValue`=7, `ReadLocalValue`=7, exactly one ordinary delivery `(100→7, Local)`, A20 `(100→250)` then `(250→7)`. Without the test the resumed write clobbers 7 with the stale 250 AND leaves an un-animated entry whose lanes disagree — `GetValue` 250 vs `GetBaseValue` 7 | PIN (PD18 × §2.2 last-writer-wins) |
| M233d *(the clear-then-rewrite residue)* | inheriting `PcDyn`-shaped property; `root.L(42)`; leaf `L(400)` with ceiling 100 (eff=100), then ceiling raised to 250 (so `coerced`=250); the observer issues `CV` **and then** `L(300)` — a different raw whose coerced result is the same 250 | `leaf.CoerceValue` | Supersession alone is **not exhaustive**: the contribution is retracted and re-established, so the stored base matches again on resume and the base test waves the operation through — but the nested writer already published the value. **Kills the no-change-test mutant.** Expected: exactly TWO ordinary deliveries, both the observer's — `(100→42, Inherited)` then `(42→250, Local)`; eff=250; `ReadLocalValue`=**300** (the nested writer's raw owns the slot, not the re-coercion's 400 — the operation is superseded wholesale, not merged); A20 sees `(100→250)`, `(250→42)`, `(42→250)`. Without the test a third, no-change `(250→250, Local)` delivery follows — the defect this row exists for, and the reason `RecoerceLocal` now carries the same equality gate every other publisher in the store has (§0.3 rule 3) | PIN (§0.3 rule 3 × PD18) |
...
## 21. Theme-reactive defaults — the `DefaultResourceKey` tier (M302–M307a)

Added 2026-08-08. `PropertyMetadata<T>.DefaultResourceKey` makes the `Default` rung *resolve*: on a `UIElement` whose resource chain answers the key, a read returns the themed value instead of the registered `DefaultValue`. It is **not a new lane** — same `Default(300)` rung, still storeless, still lazy, still beaten by every real contribution (inheritance included), still materialising neither an entry nor a subscription (`UIObject.ResolveDefaultValue`; the theme catch-all repaints the recorded readers instead, invariant 3). Neither parent framework has the tier: WPF's nearest neighbour is a `{DynamicResource}` in a theme style and Avalonia's is a theme `Setter`, and both of those are real Style-lane contributions — a `Kind == Default` probe on them answers "set", and every rung above them arbitrates against a stored value. There is therefore no external oracle to defer to, and every row here is **`PIN`** per §0.2's tag definition: no direct parent-framework analog, so this table *is* the decision record rather than a transcription of one.

What the tier costs is that `metadata.DefaultValue` stops being the value a read returns, while **every transition baseline in the store is a read**. The rows below pin exactly that: the old value leaving the tier and the new value returning to it are the RESOLVED value, so a value-bearing write of the raw default — a `SetValue(p, null)` on the ~30 keyed `IBrush?` registrations, a binding pushing a null source — is a genuine change and not an equality-gated no-op. Answered raw, those transitions self-cancel *after* the entry has already been mutated: the property reads `null` while the frame keeps painting the theme.

New notation:

- `Pk` = `StyledProperty<IBrush?>` on a `UIElement`, **no** positional default (so `metadata.DefaultValue` is `null`) and metadata `DefaultResourceKey = ThemeKeys.AccentBrush` — the shape the framework's keyed registrations share, and the one where the raw default and the resolved value are furthest apart. `Pk` does **not** inherit. `themed` = the brush the element's resource chain resolves for that key; `marker` / `over` = brushes equal to neither `themed` nor each other.
- `Pki` *(added 2026-08-09)* = the INHERITING twin of `Pk` (same metadata, `inherits: true`), read on an element under an ancestor that contributes `anc` — a brush equal to none of `themed` / `marker` / `over`. The ancestor beats the tier (M302), so every storeless-tier baseline on `Pki` is the walk-up at `Inherited(200)` rather than the themed read at `Default(300)`: `Pki` is what separates the two arms of `GetUnsetFallback`, which `Pk` alone cannot.
- `paint(v)` = the element's rendered cells actually carry `v`. These rows assert the FRAME as well as the property: the failure mode they exist for is a property and a frame that disagree, which no `eff`/`notify` cell alone catches.

| # | Setup | Operation | Expected | Oracle |
|---|---|---|---|---|
| M302 | three fixtures: a bare element carrying `Pk`; one under a contributing inheritance ancestor; and — since `Pk`'s own raw default is `null` and so indistinguishable — a `Pk`-shaped property with a *distinguishable* registered default whose key resolves nowhere | `GetValue` / `GetValueSource` / `IsSet` | bare: eff=`themed` src=`Default` (`+cur` false) IsSet=false — the resolution is a lazy READ that materialises no entry for `Pk` and takes no subscription, which is exactly what keeps the tier beaten by every rung above it (M1's shape one rung down). Ancestor: eff=the inherited value src=`Inherited` — the ladder is unchanged, the tier is still the weakest rung (M108). Unresolvable key: eff=`metadata.DefaultValue`, still neither coerced nor validated (PD8) | PIN |
| M303 | `Pk` at rest (eff=`themed`) | `L(marker)` | eff=`marker` src=`Local` IsSet=true; `notify(themed→marker, Local)` — the old value is the **themed** brush, never the raw `metadata.DefaultValue`; `paint(marker)`. The M96 shape one rung down: the old value is whatever the storeless tier had in effect, and anything that caches the payload (a `Changed` callback, an animation-handoff snapshot) reads this rather than the property | PIN |
| M304 | `Pk` at rest (eff=`themed`) | `L(null)` — a **value-bearing** write of `metadata.DefaultValue`, not `CV` (only the `UnsetValue` sentinel spells that, PD5) | eff=`null` src=`Local`; `notify(themed→null, Local)`; `paint(null)` — expressly **not** the M17/M101 equal-value gate: the baseline is the resolved brush, so writing the raw default underneath it is a real change and must fire `AffectsRender` | PIN |
| M305 | after M304 | `CV` | eff=`themed` src=`Default` IsSet=false; `notify(null→themed, Default)` (PD10); `paint(themed)` — the mirror of M304: the promotion baseline is the same read, so this is **not** the M47/M99 equal-value silent promotion | PIN |
| M306 | `Pk` | `L(marker)`, `CV`, `L(null)`, `CV` | covered by M303/M305; row asserts the pair end-to-end with src transitions `Default→Local→Default` **twice**, four notifications, none silent — the M100 pair at the themed tier, where the second round is the discriminator (`null` IS the raw default, so a raw-answered baseline makes both of its steps silent while the effective value moves) | PIN |
| M307 *(amended 2026-08-09 — the LANE travels with the baseline)* | `Pk`; `L(marker)`; a winning-base observer (A20, `IncludeBaseChanges`) that issues exactly one `SCV(over)` from its first delivery — plus a plain subscription of the same observer, since an `IncludeBaseChanges` subscription delivers on that channel only | `CV` | two base deliveries in order: `baseNotify(marker→themed, anim:false)` — the retraction, M169's shape — then `baseNotify(themed→over, anim:false)`: the re-entrant write's own baseline is the same read. The ORDINARY channel sees the write too, and the one `CV` produces **two** deliveries there, the nested dispatch completing first (PD18 — M256/M257's shape): observers receive `(marker→over, Default)`, the re-entrant `SCV`'s own, and then `(over→themed, Default)`, the outer `Reevaluate` resuming and *discarding* the overwrite; each exactly once, final eff=`themed`. **The lane travels with the baseline** *(2026-08-09)*: the `Default` on the first of those is the correction — what an `SCV` over no contribution replaces is the storeless tier it grafts over, which is M118's substitution one seam along (A11, rule 2). Never `Unset`: §0.3's sentinel is `int.MaxValue`, and leaking it is wrong in *kind* — an `== Default`/`== Inherited` or a `switch` on the args lane misses outright, and on an INHERITING property the sentinel also reads weaker than `Default(300)` where the truth is `Inherited(200)`, so threshold tests flip too. That second arm is **M307a**, not this row: `Pk` does not inherit, so `GetUnsetFallback`'s `Inherited` branch is unreachable on this fixture by construction. Note what does **not** change on this (non-inheriting) row: the corrected lane is `Default`, so a `>= Default` "nobody set this deliberately" probe answers **true** either way — the args lane was never that discriminator; per PD27 it is `Kind == Default && !IsCurrentValue`, and the `+cur` bit is set here. This is the one seam that reaches `SetCurrentValue`'s `BasePriority == Unset` arm, and PD18 is what opens it — the observer writes from inside `Reevaluate`'s retraction window, where the base lane is already `Unset` and the effective lane has not caught up. No RESTING state reaches that arm (the mouth's own guard routes an `Unset` effective lane to the local mouth, and an un-animated entry's effective and base lanes are assigned together at every mutation site), which is why it survived the tier's arrival answering `metadata.DefaultValue` raw — reporting `null` as the old base of a themed brush, skipping the inherited walk outright, and stamping the sentinel on the effective channel. One fallback now resolves both consumers, the hoist `Reevaluate` already makes | PIN |
| M307a *(added 2026-08-09 — the `Inherited` arm)* | `Pki` under an ancestor contributing `anc`; `L(marker)` on the descendant; otherwise M307 verbatim — the same two subscriptions, the same one-shot `SCV(over)` base observer | `CV` | M307 with the walk-up standing in for the themed read, and the lane with it: `baseNotify(marker→anc, anim:false)` then `baseNotify(anc→over, anim:false)`; on the ordinary channel `(marker→over, Inherited)` then `(over→anc, Inherited)`, each exactly once (PD18 order); final eff=`anc`. **This is the arm where the sentinel changes an answer, not just a kind.** M307's note holds only because `Default` is the weakest rung: `Unset = int.MaxValue` and `Default(300)` both satisfy `>= Default`, so a ranking consumer reads the same either way there. Here they do not — `Inherited(200) >= Default(300)` is **false** where `Unset >= Default` is **true**, so the leak inverts a threshold test on the very fixture the tier is designed to be beaten on. It also discriminates the fix from a hardcoded `Default`: the value the descendant reads back is the ancestor's, and the lane must name the tier that produced it (`GetUnsetFallback` returns lane and value together, so a lane that disagrees with the value is a contradiction, not a rounding) | PIN |

*Row↔test map* — all in `Cursorial.UI.Tests/DefaultResourceKeyTests.cs`, marked with their row id at the `[Fact]` (§22's `M###_slug` naming does not apply: these tests predate the rows and the rows were written to them). M302 → `BareTextBlock_ResolvesThemeTextBrush_AtDefaultTier` + `InheritedValue_BeatsTheThemeReactiveDefault` + `UnresolvableKey_FallsBackToTheMetadataDefault` (the family, per §22's `[Theory]`-equivalent allowance for M24/M114/M263-shaped rows). M303 → `LeavingAThemedDefault_CarriesTheThemedBrushAsTheOldValue`. M304 → `WritingNullOverAThemedDefault_NotifiesAndRepaints`. M305 → `ClearingBackToAThemedDefault_NotifiesAndRepaints`. M306 → `SourceTransitions_AcrossTheThemedTier_BothDirections`. M307 → `ReentrantSetCurrentValueDuringABaseRetraction_BaselinesOnTheThemedBrush`. M307a → `ReentrantSetCurrentValueDuringABaseRetraction_ReportsTheInheritedLane_WhenAnAncestorContributes`.

---

## 22. Test authoring contract

Each numbered row above becomes **exactly one** xUnit test in `Cursorial.UI.Tests`, named after its row id with a behavior slug: `M042_OnEntryChanged_InPlaceReEmit_NotifiesAtStyle` (`[Fact]`) — rows whose Expected cell enumerates a family (M24, M114, M148, M263) become a single `[Theory]` with one `[InlineData]`/`MemberData` case per family member, keeping the row↔test bijection at the row level. Tests live under `Cursorial.UI.Tests/PrecedenceMatrix/`, one file per section (`Section01_Defaults.cs` … `Section19_Allocation.cs`, `Section20_TemplateLane.cs`), sharing the §0.1 fixture via a common harness class that registers the fixture properties once (`ModuleInitializer` or static lazy — dense ids are process-global, so registrations must be idempotent across test classes). **§21 is the one exception to the file-per-section rule:** the §0.1 fixture's `Host` is a bare `UIObject`, and `DefaultResourceKey` resolution is a `UIElement`-and-resource-chain affair, so the tier is literally unreachable on that fixture — those rows live in `Cursorial.UI.Tests/DefaultResourceKeyTests.cs` on a headless host, which is also what lets them assert `paint`. Rows are not merged, reordered, or "covered implicitly by" other rows: a row without a matching test is a P0 exit-criterion failure (§14 P0: "precedence matrix green against the store"). DEBUG-only rows (M204, M255) compile under `#if DEBUG` and assert the absence of the check in release where practical. When the engine cannot honor a row, the resolution is a PR that amends this file (and, where the row carries a `PIN`/`DEV` tag, the PD ledger) **before** the engine change lands — the matrix is the oracle, not the implementation. Oracle tags (`AV`/`WPF`/`PIN`/`DEV`) are documentation of provenance and do not alter test behavior.
