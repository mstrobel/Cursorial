...
- **PD20** — A gated (comparer-equal) write does not replace the stored value (the first-stored representative survives — observable with `Pcmp`).
...
| M201 | after any instance touched `Pa` | write `GlobalEffects` / per-type effects | throws `InvalidOperationException` (registration window closed) | PIN (A1: "writable only during the registration window") |
...
| M231 | `Pc` holding eff=100 from raw 250 | `L(120)` | raw differs (250→120) but coerced result equal ⇒ `silent`; `GetValue`=100 | WPF |
