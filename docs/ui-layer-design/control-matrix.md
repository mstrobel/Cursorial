...
- ⑦ **`app.Theme.Styles` consumption (R2/B13) is LIVE** (was deferred): the `StyleEngine` now gathers `UIApplication.Theme`'s `Styles` slot at `Theme(2)` alongside the BuiltIn framework leg, and re-matches on theme reassignment *and* theme.Styles mutation (`OnThemeStylesInvalidated`, routed from the `Theme` setter + `ResourceDictionary.OnStylesMutated`). C100/C100b/C100c/C100e/C102 test the live behavior; a variant flip stays resource-only (CD15, C100e). **Two residuals** remain on C100's contract: (a) only the theme's *own* top-level `Styles` slot is consumed — Styles nested in the theme's `MergedDictionaries` are not yet flattened (the "merged-order-then-own-last" clause; a custom theme should author its `Styles` in the top-level dictionary); (b) the re-read is unconditional — the "version-compared" pulse short-circuit is a deferred optimization (functionally a no-op, only redundant work). Both are tracked follow-ups, not behavioral gaps for top-level-authored theme Styles.
