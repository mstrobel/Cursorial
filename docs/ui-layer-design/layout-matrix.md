...
| L148c | `G[cols: a,*]`, col0 `MinWidth = 8`, `MaxWidth = 3`, child `Probe(5×1)` | `layout(20×10)` | min>max ⇒ **min wins** on both the track size (`ActualWidth = 8`) and the child constraint (`= 8` every pass, never `3`) — LD18 via the single `Clamp(constraint, Min, Max)`; a two-step min-then-max clamp would let max win the constraint (3), inconsistent with the track size. Closes the Grid-track min>max gap that L9 (primitive) and L20 (element-level) don't reach | WPF |
...
| L152a | `G[rows: *]`, child whose desired REFLOWS under the final constraint (wrapping text: 1 row at the interim Unbounded width, N rows at the resolved star width) | `measure(G, bounded)` | `desired(G).Rows` reflects the POST-reflow content: after the step-4 final-constraint re-measure, step 4b resets and re-resolves content for EVERY non-Cell track (the same fresh accounting ArrangeOverride performs) — LD8's `min(resolved, content)` and Auto track sizes never read the stale interim-Unbounded measurement. Covers both legs: a star track's own content (single-star window-chrome cell) AND an Auto track on the CROSS axis grown by a bounded star column's re-wrap (the wizard's key-binding grid). Regression: fit-to-content windows clipped their wrapped text (observed live in the first-run wizard, 2026-07-06) | PIN (LD8 amendment 2026-07-06) |
...
| L202 | content `Probe(10×100)` | measure SCP at `20×10` | content's recorded vertical constraint = `U` (the sentinel pass); `Extent = 10×100`; `desired(SCP) = min(extent, constraint) = 10×10`. Greedy content (desire > `LayoutLimits.MaxScrollExtent`) is re-measured with `MaxScrollExtent` (32 000) **on the overflowing axis only** and the published `Extent` caps there | PIN (doc §12 scrolling note as amended 2026-07) |
