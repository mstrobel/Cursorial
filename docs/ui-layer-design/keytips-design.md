...
- **Exit()** (from `CueDeactivated`, activation, `ActiveWindowChanged`, or Esc-at-top): `HideKeyTipOverlay()`; clear the stack; `ResumeCue()`; if exit was NOT via a leaf activation, restore focus via the ribbon's non-retaining scope (see §7); `IsActive = false`. Idempotent (guard on `IsActive`) — critical because `CueDeactivated` and a leaf-activation Exit can race. **Active-root guard:** the restore is *suppressed* when the snapshot no longer belongs to `FocusManager.ActiveRoot` — an Alt-bearing key gesture (Alt is the drill modifier, so `Alt+N` reaches the app through the same held-Alt cue that armed the overlay) can open or switch a window before the cue-off exit fires; restoring the pre-gesture snapshot would then yank focus out of the freshly-activated window back to the old root. The new window's own `OnWindowActivated` focus must win.
...
**Built after v1 (2026-09-12, Backstage):** the File tab is a `DrillPopup` over WHATEVER its request opens — the ribbon never sees its Backstage, only that `BackstageRequested` was handled — so a drill with no `BuildNext` has the controller snapshot the surface stack before the reveal and build the level over the surface the reveal added (a FullScreen Backstage window, a Menu-mode popup, or the tab's context menu when the request goes unhandled), the level's retract closing that surface. `OnActiveWindowChanged` no longer exits blindly: it stays while a build is parked (the reveal just opened the window), pops to a held level when a window on the stack closed (Esc, the Backstage's own ◂), and exits only for an unrelated window. A File request that opens nothing ends as an activation.

**Built after v1 (2026-09-12, compose-both):** the active scope's PLAIN access-key targets (a page's check boxes, buttons, labels — anything registered with the access-key manager that is not inside a badged bar) join the overlay's ROOT level with their mnemonic as the badge (`AccessKeyManager.EligibleTargetsInActiveScope`); inline placement puts the badge where the cue would be and the cue stays suspended, so nothing draws twice. Collisions go through the one policy: an explicit key wins, colliding auto letters are suffixed (`0-9A-Z`, fixed width per group) — a content letter beside a tab letter, and content targets that would multi-match under the manager, which activate directly instead of focus-cycling. Deeper levels stay bar-only. The `:keytip-active` compose-both cue below is thereby superseded.

**Built after v1 (2026-09-13, Shift+Esc):** Shift+Esc is the exit from any depth in one stroke — the Alt pre-stage routes it to `IKeyTipController.PopAllLevels` (every drilled surface closes top down, exactly as repeated Esc would) and then through the same cue-off that Esc-at-root takes, so the overlay, the cue and the focus snapshot resolve as one exit; at the root it is simply Esc. Plain Esc keeps backing out one level at a time. The same stroke leaves a plain MENU hierarchy too (menu mode without the overlay): the sticky cue drops and — unlike plain Esc, which the manager consumes there — the key goes on to the focused menu element, so a submenu popup collapses the whole chain (decision ③) and a focused bar header leaves the bar.

**Built after v1 (2026-09-12):** the level stack follows the keyboard through menus — a submenu or sibling top-level menu the arrow keys open (its popup's placement target is one of the shown level's badged controls) gets a level pushed over its rows, a popup the arrow keys close pops its level (`KeyTipController.SyncWithPopups`, run at the post-layout hook; a keyboard-opened popup is never closed by a give-up); the QAT customize ▾ carries the explicit `00` badge and drills into its checklist; a `CheckBox` badge activation toggles the box (the click path — its own access-key reaction stays focus-first, keeping single- and multi-match consistent).
