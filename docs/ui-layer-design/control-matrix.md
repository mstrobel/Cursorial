...
- ⑦ **`app.Theme.Styles` consumption (R2/B13) is LIVE** (was deferred): the `StyleEngine` gathers `UIApplication.Theme`'s `Styles` slot at `Theme(2)` alongside the BuiltIn framework leg (with an order-base above it, so an app-theme rule **categorically overrides** an identical BuiltIn rule — a BuiltIn rule the theme does not redefine is unaffected), and re-matches on theme reassignment *and* theme.Styles mutation (`OnThemeStylesInvalidated`, routed from the `Theme` setter + `ResourceDictionary.OnStylesMutated`). C100/C100b/C100c/C100e/C102 test the live behavior; a variant flip stays resource-only (CD15, C100e). The caps-unicode/caps-nocolor styles are now authored in XAML (`Cursorial.UI.Themes.Xaml/Themes/Styles.xaml`, loaded by `CursorialXamlTheme.LoadStyles`/`LoadTheme`) — the loader's new `<ResourceDictionary.Styles>` fill (`FillStyles`), the dotted-Setter-without-`TargetType` resolution, and the `GlyphSetCarrier` string converter cover the authoring; `XamlThemeStylesTests` proves parity with BuiltIn + the override. `AccessKeyCue` stays BuiltIn-only (its `AccessKeyManager` owner is outside the default xmlns map; always supplied by the framework leg). **Two residuals** on C100's contract: (a) only the theme's *own* top-level `Styles` slot is consumed — Styles nested in `MergedDictionaries` are not flattened (the "merged-order-then-own-last" clause; author theme `Styles` in the top-level dictionary); (b) the re-read is unconditional — the "version-compared" pulse short-circuit is a deferred optimization. Both are tracked follow-ups.
...
| **R2 — built-in theme** | §4 | `CursorialTheme.BuiltIn` + `CreateDefault` + `ThemeKeys`; the tier-keyed palette + Type-keyed control themes (S8 content); the theme-styles channel armed at `Theme(2)`; `ResourceBrushResolver`; override paths. (The `Cursorial.UI.Media` builder twins were retired — #8; the real `Drawing.Media` types are element-authorable.) |
...
- **CD28 — ScrollViewer offsets are DirectProperty two-way mirrors of SCP's styled offsets** (doc §12.4, spec §3.9): `ScrollViewer.HorizontalOffset`/`VerticalOffset` (`DirectProperty`, two-way bindable) mirror `ScrollContentPresenter.ScrollOffsetColumn/Row` (**styled**, `AffectsComposite`, animatable — the doc supersedes the spec's "DirectProperty offsets, not animatable, hand-routed"). Wheel: `WheelDeltaY/120 × 3` lines, Shift/`WheelDeltaX` horizontal, unconsumed bubbles; the deltas use the Windows MIXED sign convention — positive-Y = up (toward the start), positive-X = RIGHT (toward the end) — so the horizontal lane applies the delta un-negated and Shift+wheel maps `deltaX := −WheelDeltaY` (wheel-down = right; C223b). `Auto` re-measure loop broken by remember-last-verdict. ScrollBar = 1 cell wide; rail `Pen` + `█` thumb (min 1); arrows are `RepeatButton`s. `:horizontal`/`:vertical` on ScrollBar. Doc §12.4/§12.7 over spec §3.9 (the DirectProperty/non-animatable spec stance is the inverted one). PIN/DEV.
...
- **CD30 — theme channels arm at the pinned layers** (doc §11.8, §12.1): a Type-keyed control theme (a selector-less `Style` rooted at `^`) arms at `ControlTheme(0)` wherever found in the chain; `ControlTemplate.Styles` arm at `Template(1)`; `UIApplication.Theme`'s `ResourceDictionary.Styles` (flattened merged-order-then-own-last) arm at `Theme(2)`. Element/window `Styles` slots are ignored in v1 (debug-flagged). Layer beats specificity (a P3 pin), so app styles (3+) always beat theme styles. PIN.
  - **Template-part rules live in `ControlTemplate.Styles`, not the control-theme `Style.Children`** (#115). The control theme is armed **element-addressed** (it matches its single-compound `^`/`^:pseudo` rules against the styled control itself), so a multi-compound reach-in rule (`^ /template/ part`) authored in its `Children` can never match a part — it is dropped with a one-time DEBUG diagnostic (style-matrix SD23-⑤), NOT shadowed by precedence. The part-arming pass consumes `ControlTemplate.Styles` (binding `^` to the templated parent); author `^ /template/ part` rules there (see `SliderTemplate`). Where the template factory already holds the part reference, a direct property init / `SetResourceReference` on the part is the simpler form — but note it lands at the **Template lane** (`BindingPriority.Template`, 150) rather than the **Style priority** (100) a `ControlTemplate.Styles` rule arms at, so the rule form is the weaker (more overridable) of the two; both are below LocalValue and still beaten by an App/Explicit-layer override.
...
## 4. Built-in theme + ThemeKeys + ResourceBrushResolver (R2) — C93–C112 (C104–C107 retired, #8)
...
| C102 *(amended 2026-07-12)* | layer-beats-specificity: an app style (App(3)) and a theme style (Theme(2)) both matching, the theme style more specific via a RESTING simple (`#name`) | resolve the winner | the **app** style wins (layer beats specificity) — the law holds WITHIN a slot (precedence-matrix PD26). The row also pins the cross-slot consequence: a CLASS-gated theme rule is conditional ⇒ `StyleTrigger`, and while active it beats the resting app rule despite the weaker layer (activation trumps rest, wherever each rule lives — the accepted Avalonia consequence) | PIN (CD30, P3 pin; PD26 amendment) |
...
### 4.3 ResourceBrushResolver + override paths

> **C104–C107 retired (#8).** These rows tested the `Cursorial.UI.Media` `IResourceValueBuilder` twins
> (`SolidColorBrush`/`LinearGradientBrush`/`Pen` + `.Build()`) in isolation. The twins were never wired into the
> loader and became redundant once the real `Cursorial.Drawing.Media` brush/gradient/`Pen` types were made directly
> XAML-element-authorable (#5/#20); both the twins and these rows were removed. See design §11.9 (retired) and the
> XAML brush/gradient coverage in `XamlGradientBrushTests` / `XamlValueTypeElementTests`.
...
| C223b *(added 2026-07-12 — regression pin for the reversed horizontal wheel)* | wide content, H bar visible | wheel `WheelDeltaX=+120` ×2, then `−120`, then Shift+`WheelDeltaY=+120` | `WheelDeltaX` positive = RIGHT ⇒ `HorizontalOffset` 3 → 6 → 3 (the Windows MIXED delta convention: positive-Y = up/start, positive-X = right/END — the vertical lane's negation must never be copied onto the X lane; doing so shipped a reversed horizontal wheel once); Shift+wheel maps `deltaX := −WheelDeltaY` (WPF/AV/browser parity: wheel-down = right, wheel-up = left) ⇒ offset back to 0 | PIN (CD28) |
...
| C225 | keyboard inside, focused control didn't consume: Up/Down, PageUp/Down, Home/End, Left/Right (Ctrl/Shift moves Page/Home/End to the horizontal axis; Alt/Super are not claimed) | each | ±1 row / ±viewport / extremes / ±1 col respectively | PIN (doc §12.7) |
...
| §4 Built-in theme + ThemeKeys + resolver | C93–C112 (C104–C107 retired #8) | **R2** | BuiltIn + S8-authored content + `ResourceBrushResolver` |
