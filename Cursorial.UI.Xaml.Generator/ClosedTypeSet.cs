...
internal static class ClosedTypeSet
{
    ...
    public static string? ResolveStaticExpr(XamlSymbolResolver resolver, string xmlNamespace, string memberPath)
    {
        ...
        // INSTANCE member accesses on the running value (Type.Static.Instance.Instance…) — with indexer segments
        // ([3] / ['key'] / [EnumType.Value]) anywhere after the first member, resolved through the shared
        // XamlPathParser ladder. Kept byte-identical in shape to ReflectionXamlMetadata.TryResolveStatic (X174)
        // so the lowered expression the emitter bakes and the reflected chain the loader walks cannot drift.
        return XamlPathParser.TryResolveStaticChain(resolver, xmlNamespace, memberPath, out var expression, out _)
            ? expression
            : null;
    }
