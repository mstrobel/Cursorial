...
```csharp
public enum BindingPriority   // amended 2026-07-12: the completed Avalonia lattice (PD26)
{
    ...
    StyleTrigger  =   50,  // the CONDITIONAL style slot (PD26): rules carrying any .class/:pseudo/When condition
                           //   (classLike > 0); within-slot order = Fork B's packed StyleSortKey. Above Template so
                           //   state looks pierce template-authored part values while active.
    Template      =   75,  // §20/PD24 (amended): values a CONTROL template AUTHORS on its parts (literal SetValue,
                           //   {TemplateBinding}/{Binding}, SetResourceReference) — below StyleTrigger, ABOVE resting
                           //   Style: the part's resting truth. Reached only via the template-instantiation scope
                           //   (DataTemplate content stays LocalValue). Wire moved 150 → 75 per the §2.9 gap contract.
    Style         =  100,  // the RESTING style slot: purely structural rules (types/#names/combinators//template/ hops);
                           //   within-slot order = the same packed StyleSortKey (layer beats specificity)
...
- **Two style slots (the activator split — PD26, 2026-07-12).** Every style contribution is a frame; the frame's slot is fixed at construction from the rule's SHAPE: conditional (`CompiledRule.IsConditional ⇔ classLike > 0` — any `.class`/`:pseudo` simple on any compound, or a `When` condition) ⇒ `StyleTrigger`; purely structural ⇒ resting `Style`. THE SLOT BEATS THE SORT KEY — an active conditional rule beats every resting rule regardless of layer or specificity (Avalonia's model: activation trumps rest; an activated theme rule beats a resting app rule — accepted; the unconditional per-element override is `SetValue`). Within each slot, frames order by `StyleSortKey` (Fork B owns key construction; layer beats specificity per DECISIONS). The store sorts frames by the key, filters by slot, and arbitrates; it never evaluates selectors. Frame-hosted binding entries inherit the host frame's slot (A5).
- **The Template lane** (`BindingPriority.Template`, wire 75 — added 2026-06-16, amended 2026-07-12; precedence matrix §20/PD24) is the engine-internal lane for everything a **control** template *authors on its parts*: a literal `SetValue`, a `{TemplateBinding}`/`{Binding}`, a `SetResourceReference`. It sits **below** the conditional `StyleTrigger` slot (state looks pierce authored part values while active, and retract cleanly to them) and **ABOVE** the resting `Style` slot (a template author's literals and TemplateBinding plumbing are the part's resting truth — a broad structural rule cannot wreck template wiring; re-skinning at rest styles the CONTROL's own properties through the forwarding spine, or uses conditional rules). It is **not** a `SetValue` priority argument (PD1 stands); a thread-static **template-instantiation scope** open while a control template's content tree is built (`ControlTemplate.Instantiate`, `ItemsPanelTemplate.Build`) reroutes the ordinary local-lane producers to it — **`DataTemplate.Build` deliberately does not open it** (data-template content is app content at LocalValue: authoring-equivalent to hand-writing the tree at the use site; PD24 amended). **Do not confuse it with Fork B's `StyleLayer.Template`** (§3.4): that is a *sort-key layer within the style slots* for style rules declared in a control template's `Styles` — such a rule arbitrates in whichever SLOT its shape selects, like any other rule. (A `BindingEntry<T>` installed inside the scope captures the lane at install — A6 accepts `LocalValue` or `Template`.) *History:* the 2026-06-16 half-adoption placed the lane below ALL styles; completed into the Avalonia lattice (`StyleTrigger > Template > Style`) on 2026-07-12 — see §20's header.
- **`SetCurrentValue<T>`** (verbatim P3 graft; provenance amended 2026-07-12, PD27): replace the effective value in place without changing its source; no entry ⇒ grafts as local **storage** (shadows the subtree, evaporates on a producer's arrival) while `GetValueSource` reports the UNDERLYING source it overlays (`Default`/`Inherited` `+cur` — WPF parity; `Kind == Default && !IsCurrentValue` is the "untouched" test) and `ReadLocalValue` does not see it (M264c). Observer args carry the priority of the lane whose value was replaced — `Animation` while an animation holds the property, else the base lane's priority; and **whenever that base lane resolves to nothing, the storeless tier underneath it** — the inherited walk-up else the (possibly themed) `Default` tier, i.e. exactly what a read would answer, never the internal `Unset` sentinel (A11 as widened 2026-08-09; matrix §0.3 rule 1). The pure graft is the resting instance of that rule (`Default`/`Inherited`, PD27, M118/M127); the re-entrant instance is a base observer that writes from inside `Reevaluate`'s retraction window, where the base lane is already `Unset` and the effective lane has not caught up (PD18; M307 for the `Default` arm, M307a for the `Inherited` one). `CoerceValue`'s re-coercion notification, which reads its lane after its own base dispatch, reaches the same window but is closed the other way — at the source: **an operation must publish only a change that is still its own and still a change**, so a re-coercion whose base observer clears, re-writes, or clears-then-rewrites simply stops, and the delivery that leaked the sentinel there no longer happens (matrix M233a–M233d, 2026-08-09). `ClearValue` undoes `SetCurrentValue` universally (M125 amended): it clears the graft AND strips a `+cur` overlay riding a producer lane, restoring the lane's source value. Non-echo `SetValue` **and** `SetCurrentValue` writes feed TwoWay write-back (A12 — `Popup.IsOpen` dismissal depends on it); jointly: a mid-animation `SetCurrentValue` on a two-way-bound property never reaches the source, because S2 filters Animation-priority args (pinned as one oracle row, §2.5).
...
- **Resolution:** local → active `StyleTrigger` frames (StyleSortKey order, later/stronger first) → the Template lane → active resting `Style` frames (same ordering) → inherited walk (if `Inherits`) → per-runtime-type metadata default. Frame entries with `HasValue == false` are skipped (unset promotion).
...
- **WPF's 10-bucket priority ladder** — unused at terminal scale; cut rungs recorded as re-addable. The first re-add landed 2026-06-16: the **Template lane** (`BindingPriority.Template`, §2.2/§20), initially below all styles; the second landed 2026-07-12: **`StyleTrigger`** (the conditional style slot, PD26), completing the Avalonia lattice `StyleTrigger > Template > Style` (WPF puts template values above ALL styles — not adopted). Template's wire value moved 150 → 75 under the deliberate-gap contract (ordering, not magnitude, is the API promise).
...
| A11 | `SetCurrentValue` observer-args priority | S2 | Args carry the replaced lane's priority — `Animation` under animation, else the base lane; when that base lane resolves to nothing, the storeless tier underneath it (`Inherited` if an ancestor contributes, else `Default` — what a read would answer), **never** the `Unset` sentinel *(widened 2026-08-09; matrix §0.3 rule 1, rows M118/M127 at rest and M307/M307a re-entrantly)* |
...
> Decision: **hybrid** — a deliberately small CSS-style selector subset as the single *structural* activation mechanism, plus `When` data-conditions (the DataTrigger equivalent) as the single *non-structural* one. A style is active on an element iff **(structural selector matches) ∧ (all required pseudo-classes set) ∧ (all `When` conditions true)**. No `Trigger`s, no trigger priority slots, no property-value selectors. One predicate, one packed sort key, one cookie-based retraction path — and since 2026-07-12 TWO store slots selected by rule shape (`StyleTrigger` for conditional rules, resting `Style` for structural ones — the Avalonia activator split, precedence-matrix PD26; the predicate and machinery are unchanged, only the arbitration tier differs). Canonical proposal: `proposal-styling-hybrid.md`, amended per the ledger in §3.11.
...
Style values enter Fork A in one of TWO slots selected by rule shape (PD26, 2026-07-12): conditional rules (any `.class`/`:pseudo`/`When` — `classLike > 0`) at **`BindingPriority.StyleTrigger`**, purely structural rules at resting **`BindingPriority.Style`** — the full ladder is `Animation > LocalValue > StyleTrigger (within-slot StyleSortKey) > Template > Style (within-slot StyleSortKey) > Inherited > Default`. Values a **control** template *authors on its parts* (`{TemplateBinding}`, literals, `SetResourceReference`) enter at **`BindingPriority.Template`** BETWEEN the slots (§2.2/§20 as amended): an active conditional rule pierces an authored part value; a resting rule — whatever its layer, `StyleLayer.Template` included — cannot (the authored value is the part's resting truth; re-skin through the control's own properties or conditional rules). The `StyleLayer.Template` layer and the `BindingPriority.Template` lane are **different mechanisms that share a word** — the former orders rules *within* a style slot, the latter is the authored-part lane *between* the slots.
...
| B15 | Binding-valued setters frame-hosted | S2 | `BindingOperations.Install(element, property, binding, hostFrame)` with the engine's own `ValueFrame`; cookie retraction evicts; free-standing `Bind` is LocalValue-only. **Landed:** `StyleSetterConverter` passes a `BindingBase` through seal verbatim and `StyleRuleFrame.OnInstalled` installs one frame-hosted expression per styled element, so a single authored descriptor serves every element the rule matches. The frame **is** the cookie — the expression `Install` returns is deliberately dropped, because `ValueStore.RemoveFrame` evicts the hosted entries, whereas disposing them from an `OnRemoving` leg would unhook each entry while the store is still attached and the eviction lifecycle would never run. Installed once at frame install and left live across deactivation (PD3 — an activation toggle re-arbitrates, never evicts), matching B16's watcher rule. A binding setter declares **no** entry in the frame's array; the hosted entry is the whole contribution. Two seal-time guards: the `BindingBase` check sorts **above** the assignability rung, so a descriptor is never taken as a literal on an `object?`-typed target (`DataContext`, `ContentPresenter.Content`, `ButtonBase.CommandParameter`); and a direct-property target is a seal error, since `BindingExpressionCore.Lane` tests `IsDirect` before the host frame and would write with no entry for retraction to evict (A24). Authorable as `Value="{Binding …}"` in **both** XAML lanes — descriptor, never an install — and code-first via `Style.SetBinding`. `{TemplateBinding}` in a setter stays CUR2401 (ambiguous templated parent outside a template body). |
...
**Precedence:** the loader is a well-behaved value source, never a precedence authority. Document-level sets carry `BindingPriority.LocalValue`; values authored inside a CONTROL-template build land on the **`BindingPriority.Template`** lane (§2.2/§20 as amended 2026-07-12) — below the conditional `StyleTrigger` slot (state rules pierce), ABOVE resting `Style` (a document-level `LocalValue` set still overrides; a resting style rule does not — the authored part value is the resting truth). Data-template content lands at `LocalValue` (PD24 amended — app content). Within-slot ordering is Fork B's `StyleSortKey`; restoration is store-owned (invariant 4) — the loader retracts nothing.
...
4. **No font converters** — no WPF font-object model / 100–900 numeric weights. *(Refined 2026-07-13, proposal-textattributes-decomposition: the aggregate `TextElement.TextAttributes` flags property was decomposed into per-axis non-inheriting properties — `TextWeight{Normal,Faint,Bold}` / `TextStyle{Normal,Italic}` / `Underline:UnderlineStyle?` / `Inverse` / … — so the lattice arbitrates each axis independently; the "no font types" spirit stands, the deviated names signal the deviated domain.)* FIGlet/`TextSizing` properties as before.
...
**Route building (amended 2026-07-19 — the input-routing adversarial review):** visual-parent walk target→surface root into pooled scratch (free-list for nested dispatch); **at a surface root the route continues via the LOGICAL parent only** (popup root → `Popup` element → host chain) — Esc inside a menu closes the popup, not the window (pinned test). The `PlacementTarget` leg is a hierarchy/lookup bridge, **never** a route (WPF's `IgnoreModelParentBuildRoute` shape): a standalone popup's route ends at the `Popup` element, and its owner's raw handlers never see popup input (`StandalonePopup_KeysContained_OwnerHandlerNotDelivered`); owner/window/app **chords** still fire via the step-4b gesture TAIL (an `InputBindings`-only sweep continuing along the ownership chain past the route end — placement owner chain, then window root → app root; KeyDown-only per ND9, inside the pooled-args scope, inert under modal-blocked windows). The **pointer family** (`Mouse*`/`Wheel`/`QueryCursor`) is `SurfaceScoped` — its routes are `VisualParent`-only and never leave the hit surface (completing the 13b34bb hover confinement; the dead-space toggles, wheel-under-popup, owner-ForceCursor, and cross-seam right-click hazards die with the crossing). `GotFocus`/`LostFocus` are focus-STATE notifications raised along the OWNERSHIP chain (`VisualParent ?? UIParent` — full placement reach, coherent with their `IsKeyboardFocusWithin` gates). KeyUp shares the key chain (audited: no cross-seam KeyUp consumer exists). The `TemplatedParent` route leg is provably dead and debug-asserted. Ownership walks (focus chains, access keys, capture, dismissal ancestry, `IsAncestorOf`, binding anchors) deliberately keep `VisualParent ?? UIParent` — relations, not routes. Handler exceptions propagate to S6's drain (fail fast); pooled resources/interaction scopes are `using`-protected along the unwind. Nested `RaiseEvent` legal; `ProcessEvent` re-entry is a debug assert.
...
**Post-Escape suppression (ESC-framing hazard).** A lone `ESC` byte is ambiguous — Escape, or the prefix of an Alt chord (`ESC 'o'` ≡ `Alt+O`) — and S6's pump resolves it with a timing race (`VtInputDevice.DefaultEscapeAmbiguityTimeout`, 50 ms). A user who presses Esc and immediately types a letter straddles that race, so the framing the app observes for the *same* keystrokes depends on read batching; in a file dialog whose Esc moves focus from the breadcrumb to a type-ahead list, `Alt+O` ≡ *Open* commits the dialog on a keystroke meant as type-ahead. Guard: for one `AccessKeyManager.EscapeSuppressionWindow` (default = the ambiguity timeout; adjustable, `Zero` disables) after a **bare** `Key.Escape` was **delivered** to the tree, an Alt chord with **no live Alt bracket** — precisely the shape ESC framing fabricates — does not activate. Narrow by construction: a bracket-corroborated chord still activates inside the window, unmodified menu-mode letters and F10 are untouched, an Esc the pre-stage *consumed* (KeyTip level, sticky menu mode) never arms, the key still routes normally, and the cue never changes state — the check sits **before** the chord-flash self-correction so an untrusted chord cannot latch sticky menu mode. The arm is a `TimeProvider` timestamp on the application clock (fake-clock deterministic) and is dropped by renegotiation and terminal focus-out.
...
Exactly one `ReadAllAsync` enumeration per session (single-shot contract). Assembly: `host.Input` → optional `KeyReleaseSynthesizer` (opt-in, innermost) → optional `WithNumpadKeyTranslation` → `WheelAxisLock` (**always** assembled: the chain is single-shot, so the live `ScrollDeadZoneEnabled` toggle — the `input.scrollDeadZone` user option — must find the filter in place; disabled = pass-through) → `WithClickSynthesis` (S3-contract defaults, outermost). The pump `Task.Run`s the pull enumeration into a `ConcurrentQueue<InputEvent>` + `Wake()`; EOF ⇒ shutdown; faults land in an `Interlocked` slot surfaced **once** on the UI thread. **The pull surface is used, never `EventInputDevice`** (it swallows handler exceptions; S6 owns exception visibility). S6 never disposes the decorated device — `host.DisposeAsync()` owns transport lifecycle (the session drains trailing reports through it).
...
- **In a `Setter`:** the styling engine subscribes at frame-arm/first-activation; the resolved value lives inside the frame's entry and rides the owning frame's SLOT (`StyleTrigger` for conditional rules, resting `Style` for structural ones — PD26) at its `StyleSortKey` (ControlTheme(0) / Template(1) / Theme(2) / app layers). A pulse **mutates the entry in place** (`OnEntryChanged`) — never frame removal/re-add, so no re-match, no sort churn. Frame deactivation calls `Pause()`; activation calls `Resume()` **before the frame's entries are read** (pinned Fork B ordering); frame disarm — including element detach — disposes. Both Pause/Resume are O(1) allocation-free flag writes because they ride the `:pointerover` hot path under any-event motion.
