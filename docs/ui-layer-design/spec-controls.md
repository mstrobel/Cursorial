...
- Keyboard (when focus is inside and the focused control didn't consume): Up/Down ±1 row, PageUp/PageDown ±viewport, Home/End vertical extremes, Left/Right ±1 col; Ctrl/Shift pairs PageUp/PageDown and Home/End with the HORIZONTAL axis. Vertical arms require no modifier at all, so Alt/Super-modified keys bubble on rather than being consumed. `EnsureVisible(rect)` scrolls minimally; ListBox/TextBox call it for focused container/caret.
