...
{
    public static LoweringResult? Emit(XamlDocument document, string xamlPath, string xamlRelativePath, XamlSymbolResolver resolver)
    {
        ...
        if (rootClass is { Length: > 0 })
            return EmitCodeBehind(document, xamlPath, xamlRelativePath, resolver, rootClass, named);
        ...
        if (IsResourceDictionaryRoot(document))
            return EmitResourceDictionaryBuilder(document, xamlPath, xamlRelativePath, resolver);

        return null;
    }

    // The X4.6/X5 x:Class path: a partial class with typed x:Name fields + InitializeComponent.
    private static LoweringResult EmitCodeBehind(XamlDocument document, string xamlPath, string xamlRelativePath, XamlSymbolResolver resolver, string rootClass,
    ...
    private static LoweringResult EmitResourceDictionaryBuilder(XamlDocument document, string xamlPath, string xamlRelativePath, XamlSymbolResolver resolver)
    {
        ...
        string pathPrefix = Path.GetDirectoryName(xamlRelativePath)
                               ?.Replace($"{Path.DirectorySeparatorChar}", "")
                                .Replace($"{Path.AltDirectorySeparatorChar}", "")
                            ?? "";

        var method = "Build" + ToPascalIdentifier(pathPrefix + Path.GetFileNameWithoutExtension(xamlPath));
