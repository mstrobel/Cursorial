...
public sealed class Section13_DeferredContentBuild : LoaderTestBase
{
    ...
    private static UIControls.Button ApplyTo(ControlTemplate template)
    {
        template.TargetType ??= typeof(UIControls.Button);
        var owner = new UIControls.Button { Template = template };
        owner.ApplyTemplate();
        return owner;
    }
    ...
    public void X153_ControlTemplate_BuildsFreshSubtree()
    {
        ...
        var owner = new UIControls.Button();
        var instance = InstantiateOn(template, owner);
        var border = Assert.IsType<UIControls.Border>(instance.Root);
    ...
    public void X154_DoubleBuild_DistinctInstances()
    {
        ...
        var first = InstantiateOn(template, new UIControls.Button());
        var second = InstantiateOn(template, new UIControls.Button());
    ...
    public void X156_TemplateScope_CarriedOnTemplatedParent()
    {
        ...
        var part = scope!.RequireControl<UIControls.Border>("PART_Border");
    ...
    public void X157_DocumentChild_vs_TemplatePart_NoLeak()
    {
        ...
        var docRoot = Load<UIControls.Border>("<Border x:Name=\"Shared\"/>");
    ...
    public void X158_DocumentLocalValue_OverridesTemplateShipped()
    {
        ...
        var owner = new UIControls.Button();
        var instance = InstantiateOn(template, owner);
        var border = (UIControls.Border)instance.Root;
        // The template-built value is present at LocalValue (the loader's value-source).
        Assert.Equal(BindingPriority.LocalValue, border.GetValueSource(UIControls.Border.BackgroundProperty).Priority);
    }
    ...
    public void X161_TargetTypeMismatch_Throws()
    {
        ...
        Assert.Throws<InvalidOperationException>(() => template.Instantiate(new UIControls.CheckBox()));
    }
    ...
    public void X164_TemplateOwnResources_ShadowDefinitionSite()
    {
        ...
        var template = (ControlTemplate)border.Resources["T"]!;
        var instance = InstantiateOn(template, new UIControls.Button());
        var part = (UIControls.Border)instance.Root;
    ...
    {
        var block = Load<UIControls.TextBlock>("<TextBlock Text=\"_Run\"/>");
    ...
    {
        var label = Load<UIControls.Label>("<Label Content=\"_File\"/>");
    ...
    {
        var button = Load<UIControls.Button>($"<Button Content=\"{content}\"/>");
