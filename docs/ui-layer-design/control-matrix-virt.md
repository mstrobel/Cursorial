...
| **VV1.5** | The 32K cap is lifted under a host (up to the `Rect` ceiling). | A host `GetExtent()` above `MaxScrollExtent` (32K) publishes past the legacy cap — `SCP.Extent`/`ScrollViewer.Extent`/`ScrollBar.Maximum` reflect it and the offset reaches the tail (`Extent − Viewport`). **The ceiling is `Rect.MaxDimension` — now `int.MaxValue / 2` (≈ 1.07 B cells) after the Int32-`Rect` refactor**, effectively unbounded for any real list (the content is arranged at the full extent height; the half-int cap keeps `Rect` edge arithmetic from overflowing and stays distinct from the `Unbounded` sentinel). The legacy (non-host) path keeps the lower `MaxScrollExtent` sanity cap + its diagnostic. |
...
forwards `false` because no shipped panel is a host); (b) the host extent must cap at `Rect.MaxDimension` (the content
is arranged at the full extent height, so an over-`Rect` extent throws at arrange — now `int.MaxValue / 2` after the
Int32-`Rect` refactor, effectively unbounded; VV1.5/VV1.5b); (c) the viewport-constraint measure is a behavioral fork for any intervening template element
