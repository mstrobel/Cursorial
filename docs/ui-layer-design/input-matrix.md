...
- **ND17 — directional scoring** (doc §7.7 made exact): candidates filtered by direction on window-translated bounds (`TranslateToWindow`; e.g. Up ⇒ candidate bottom edge strictly above current top edge); score = facing-edge distance + 2 × orthogonal-range gap (gap 0 **iff** cross-axis ranges overlap; disjoint ranges score separation + 1, so a touching range is misaligned — 2026-07-08 clarification, N132a: touching previously scored 0 and tied same-row candidates, which broke Left/Right on a two-row ribbon stack's second row); lowest wins; ties → tab order. `Cycle` wraps to the farthest candidate on the opposite side; `Contained` stops; handled only when focus moved. PIN.
...
| N116 | B focused, memory recorded | `OnWindowDeactivated(Root)` then `OnWindowActivated(Root)` | deactivate clears `ActiveRoot`; re-activate restores focus to B from memory with `Restore` (validated); if B was disabled meanwhile → first tab-ordered instead. (Nested-scope case — focus deep in a sub-scope — restores the exact element: N227) | WPF-shaped; PIN |
...
| N132a *(added 2026-07-08)* | two equal-facing right-side candidates: one on the SAME row, one on the ADJACENT (touching) row, document-earlier | RightArrow | the same-row candidate wins — gap 0 **iff the cross-axis ranges overlap**; disjoint ranges score separation + 1, so touching = 1, never a tie. (The two-row ribbon-stack regression: touching and overlapping both scored 0, and the adjacent row's document-earlier control won the tie — Left/Right on a stack's second row yanked focus to the first.) | PIN (ND17 clarified) |
...
| N227 | focus on a leaf inside a NESTED focus scope (a `ListBox` items host) so the root scope's own memory points at a DIFFERENT, earlier-focused root-scope element | `OnWindowDeactivated(Root)` then `OnWindowActivated(Root)` (a modal dialog opening then closing, an alt-tab) | re-activation restores the EXACT element that had focus, not the root scope's stale direct-focus memory: `OnWindowDeactivated` captures the live `FocusedElement` into the root scope's memory (only when it is genuinely within the root) so N116's "restore the focused element" holds for nested scopes too. The captured value is a leaf, so `OnWindowActivated`'s restore (and any scope-entry resolution) returns it directly — no drill, no loop. The inner scope's own memory survives. Amends the Phase2 round-trip test (which previously asserted the root-scope tab stop) | DEV (extends N116; bars/nav rework 2026-06-30) |
| N228 | `FocusManager.FindNext(from, direction)` for: a non-null `FocusNavigationDirection` inside a `DirectionalNavigation` container; `direction: null`; a non-null direction with NO directional container | query (never moves focus) | non-null direction routes through the geometric scorer `NextDirectional` (the rightward neighbor, Cycle-wrapping per the container) — NOT the tab walk; `null` is identical to the no-arg `FindNext(from)` tab-order form (backward-compatible); a non-null direction with no `DirectionalNavigation` container returns `null` (opt-in, N135) while the tab form still finds the next stop. The directional branch is the `BarLabel` forwarding path (N229) | DEV (bars/nav rework 2026-06-30) |
| N229 | a `BarLabel "_Edit:"` titling a cluster on a `Toolbar`, focus on a pre-bar editor; variants: single match; two same-mnemonic captions; a TRAILING label (nothing focusable to its right); a disabled label | activate the folded mnemonic (Alt+E) through the `AccessKeyManager` | a `BarLabel` is an `IAccessKeyTarget` whose `Content` folds (`ParsesAccessKeyLiterals`, inherited `ContentControl` registration) and on a SINGLE match forwards focus to the next control to its RIGHT (`FindNext(this, Right)`) with **`method = AccessKey`** — a one-shot mnemonic, so a subsequent invoke inside the non-retaining `Toolbar` auto-returns to the editor (Directional would strand it). **Multi-match** ⇒ no forward (`if (e.IsMultiMatch) return;`, ND18 — the manager owns cycling, mirroring `Label`). A **trailing** label whose rightward query Cycle-wraps to the leftmost control is a **no-op** (forward only to a control strictly to the right). A **disabled** label is access-key-ineligible (`IsAccessKeyEligible`) ⇒ no match, no forward | DEV (bars/nav rework 2026-06-30; review follow-up) |
