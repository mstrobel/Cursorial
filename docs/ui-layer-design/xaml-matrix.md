...
| X86 *(amended 2026-07-13)* | `Color="#66d9ef"`, `"#fff"`, `"#ff000080"` (RRGGBB**AA**) (one `[Theory]`) | convert | hex parse delegates to `Color.TryParseHex`; the 8-digit form is `#RRGGBBAA` — alpha **last** (one convention across the stack: `FromRgba`/`MarkupColor`/`StyleDiagnostics.FormatValue`), a deliberate **DEV** from WPF's `#AARRGGBB` (proposal-textattributes-decomposition §10) | DEV (XD13; was PIN) |
...
| X160 | `TemplateBinding` inside a build `<ControlTemplate><Border Background="{TemplateBinding Background}"/></ControlTemplate>`, owner `Button` with `Background=Red` | `Build` | the part's `Background` tracks the owner's `Background` (one-way to templated parent, applied at build via `TemplateBuildContext.TemplatedParent`); a non-template `{TemplateBinding}` is X56. **Amended 2026-07-15 (X160a):** a TYPE-QUALIFIED source — `Owner.Prop`, WPF's optional-parens `(Owner.Prop)`, or the xmlns-prefixed `ns:Owner.Prop` — names its declaring type explicitly (an attached property, or one declared on a base): `ResolveTemplateBindingSource` splits the owner off the last dot and resolves it through the prefix-aware `TryResolveTypeToken`, then finds the property on that owner (an unresolvable owner is `CUR2002`/`UndeclaredPrefix`, an unknown property `CUR2102`, both positioned at instantiation). The closed-set sweep collects the qualified owner so the generated provider bakes it. | PIN (doc §4.4) |
