...
- **SD9 — P3 setter-value vocabulary.** A setter value is a constant or `UIProperty.UnsetValue`. Constants are validated and converted **once at seal**: exact/assignable types pass through; `IConvertible` primitives convert via invariant-culture `Convert.ChangeType`; `null` is legal for reference/nullable property types; anything else is a seal error naming (style, rule index, property). An `UnsetValue` setter compiles to a **valueless entry** (Fork A ledger A8 — contributes nothing while active; the P4+ resource-pulse semantics reuse the same entry shape). `ResourceReference` (B10) and `BindingBase` (B15) setter values have since joined the vocabulary as **descriptors**: both skip constant conversion at seal and are resolved per styled element by the frame — a `ResourceReference` against the element's resource chain, a `BindingBase` through a frame-hosted `BindingOperations.Install`. Both descriptor rungs sort **above** the assignable-type rung, which is load-bearing rather than cosmetic: `BindingBase` is assignable to `object?`, so on an `object?`-typed target (`DataContext`, `ContentPresenter.Content`, `ButtonBase.CommandParameter`) a lower placement would take the descriptor as the literal value — no error, no evaluation. A `BindingBase` targeting a **direct** property is a seal error: it has no store ladder to host the entry, so nothing would evict on retraction (A24).
...
- **SD13 — the `Explain` line format (the §3.9 acceptance contract).** One line per contributor, strongest first; format: `<Property> = <value> <- <Layer>(<n>) "<selector>" names=<n> classLike=<n> types=<n> depth=<n> order=<n> key=0x<16-hex> -- <status>` where `<value>` is the entry value's invariant-culture `ToString()` (`(unset)` for valueless entries), `<selector>` is the flattened canonical selector text (`(explicit)` for selector-less explicit styles), the hex is the full packed key (16 digits, upper-case), and `<status>` is `winning` or `shadowed`. When a lane outside BOTH style slots holds the effective value, the first line is `<Property> = <value> <- <Lane>` (`LocalValue`/`Animation`/`Template`/`Inherited`/`Default`) and every style line is `shadowed`. Explain renders **active** contributors (winning + shadowed) plus the stronger-lane line; armed-inactive rules appear in `MatchedRules`, not `Explain`. *(2026-07-12: contributors order slot-first — every `StyleTrigger` frame before every resting `Style` frame (the slot beats the key), strongest-first within each slot; the line format is unchanged. A Template-lane value masking only the RESTING slot does not render a stronger-lane line when a trigger contributor wins.)* *(Same-day audit fix: `MatchedRules` also renders arbitration order — trigger rules before resting ones, then key — and `MatchedRuleInfo` gained a `Priority` field carrying the slot. A selector LIST whose members differ in conditional-ness splits across slots — each member keeps its own key AND its own slot; S79 re-pinned.)*
...
| S38 *(re-pinned 2026-07-12)* | `a.Style = {P=9}` (selector-less) + a RESTING scoped rule `Widget#save{P=1}` + a conditional scoped rule `Widget.a{P=3}` (class toggled) | `eff` | `9` while the conditional rule is inactive — Explicit(5) beats resting Scoped at any specificity; `3` (`src=StyleTrigger`) once `.a` is present — an ACTIVE conditional rule pierces the resting Explicit (PD26: slot beats key; the unconditional per-element override is `SetValue`) | AV (activator lattice); the within-slot law stays WPF/AV |
...
| S71 | `R("Widget:pointerover"){P=5}`; `a` not hovered | `arm(a)`, then `flip(a, PointerOver, on)` | armed + `IsActive == false`, `src=Default`; flip ⇒ active, `eff=5`, `src=StyleTrigger` (conditional slot) — the armed-vs-active split, no re-match on the flip | PIN (doc §3.3 Phase 1/2) |
...
| S88a | a control with a `Theme` override whose **`ControlTemplate.Styles`** carries `^ /template/ Widget {P=5}`; the template roots a named `Widget` part | show + arm(part) | armed + active at `Template(1)`; `eff=5` — the sanctioned home for part rules (CD30). The same form authored in the control-theme `Style.Children` does NOT (S88b) | PIN (SD8/CD30; #115) |
| S88b | the SAME `^ /template/ Widget {P=5}` placed in the control-theme **`Style.Children`** (element-addressed) instead of the template's Styles | show + arm(part) | **no match** — zero frames, `src(P)=Default` (not a precedence loss; an element-addressed channel matches the subject against the control itself, so a multi-compound reach-in rule can never match). One-time DEBUG diagnostic (SD23-⑤) | PIN (SD8/CD30; #115) |
...
| S91 | `.a{P=1}` active + `.a:pointerover{P=2}` active (hovered) | `flip(a, PointerOver, off)` | `notify(2→1, StyleTrigger)` (both rules class-gated ⇒ both in the conditional slot) — runner-up promotion in ONE notification; no Default flash, no intermediate value | invariant 4; AV (M39) |
...
| S94 | stronger rule active; weaker rule activates (its pseudo flips on) | notifications | silent for the masked property; `GetValue(P, StyleTrigger)` still resolves the winner (both rules conditional) | AV (M37) |
| S95 | weaker rule active; stronger rule activates | notifications | `notify(weak→strong, StyleTrigger)` (conditional rules) | AV (M38 spirit) |
...
| S100 | `SCV(7)` then style rule `{P=5}` activates | `eff`/`src` | `5`, `src=StyleTrigger` (a `:pointerover`-gated rule) — a producer change replaces the current-value overlay (A11 semantics) | AV |
| S101 | winner `{P=5}` + runner-up `{P=5}` (equal values, both class-gated) | deactivate the winner | silent (equal-value promotion); `src` stays `StyleTrigger` | AV (PD9 within-slot) |
