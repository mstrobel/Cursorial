...
public class DrawingContextTintTests
{
    ...
    /// <summary>
    /// Paints <paramref name="seedStyle"/>-styled "X"s over the whole scene, then runs
    ...
    /// </summary>
    private static Scene Tinted(int columns, int rows, CellStyle seedStyle, Action<DrawingContext> tint)
    {
        ...
        scene.Draw(context =>
        {
            ...
            for (int column = 0; column < columns; column++)
                context.Set(column, row, "X", seedStyle);
