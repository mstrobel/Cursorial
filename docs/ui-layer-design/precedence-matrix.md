...
## 15. Coercion and validation (M230–M243, plus M233a)
...
| M233a *(added 2026-08-09 — the second sentinel leak)* | an INHERITING `PcDyn`-shaped property; `root.L(42)`; on the leaf `L(250)` with ceiling 100 (eff=100), then ceiling raised to 300; a winning-base observer (A20) on the leaf that issues exactly one `CV` from its first delivery, plus a plain subscription of the same observer | `leaf.CoerceValue` | "priority = current effective lane" (M233) is read from `EffectivePriority` **after** the re-coercion has already dispatched its A20 base change — a user callback (PD18), so the lane can move underneath it. Two ordinary deliveries, the nested one completing first: `(100→42, Inherited)` — the re-entrant `CV` retracting the last contribution — then `(0→250, Inherited)`, the re-coercion resuming and announcing its own value; each exactly once. The **second** lane is the row: `ClearLocal`'s `Reevaluate` has set `EffectivePriority = Unset`, and reported raw that is §0.3's `int.MaxValue` sentinel on a notification. It answers the storeless tier instead — the same substitution `SetCurrentValue`'s `Unset` arm makes (M307/M307a), reached the other way. No RESTING state takes it: `HasLocal` is the mouth's own guard and an un-animated entry holding a local contribution carries `LocalValue` on both lanes | PIN (PD18 × A11; M233 unchanged at rest) |
