...
public sealed class UserOptionsStore
{
    ...
    public static UserOptionsStore Load(string applicationId, IUserConfigurationPathProvider? pathProvider = null)
    {
        ...
        catch (Exception ex)
        {
            ...
            fallback.AddDiagnostic($"The configuration path provider failed ({ex.GetType().Name}: {ex.Message}); using defaults.");
            return fallback;
        }
    ...
    /// <summary>The effective value: the per-app overlay when it explicitly sets the key, else the global value, else <see langword="null"/>.</summary>
    ...
    /// <summary>The effective value parsed as a boolean (<c>"true"</c>/<c>"false"</c>, case-insensitive); <see langword="null"/> when unset or unparseable.</summary>
    public bool? GetBoolean(string key)
        => GetString(key) is { } text && bool.TryParse(text, out var value) ? value : null;

    /// <summary>The effective value parsed as an invariant integer; <see langword="null"/> when unset or unparseable.</summary>
    public int? GetInt32(string key)
        => GetString(key) is { } text
    ...
    public void SetGlobal(string key, string? value) => Set(_global, key, value);
    ...
    /// </summary>
    public void SetApplication(string key, string? value) => Set(_application, key, value);

    /// <summary>Sets a global boolean (<c>"true"</c>/<c>"false"</c>).</summary>
    public void SetGlobal(string key, bool value) => SetGlobal(key, value ? "true" : "false");

    /// <summary>Sets a per-app boolean (<c>"true"</c>/<c>"false"</c>).</summary>
    public void SetApplication(string key, bool value) => SetApplication(key, value ? "true" : "false");
    ...
    internal void AddDiagnostic(string message) => _diagnostics.Add(message);
        ...
        try
        {
            ...
            foreach (var property in document.RootElement.EnumerateObject())
            {
                switch (property.Value.ValueKind)
                {
                    ...
                    default:
                        AddDiagnostic($"'{path}': key '{property.Name}' has an unsupported value kind ({property.Value.ValueKind}); ignored.");
                        break;
                }
            }
        }
    ...
    private static void WriteFileAtomic(string path, Dictionary<string, string> values)
    {
        ...
        try
        {
            ...
            File.Move(temp, path, overwrite: true); // same-directory rename — atomic on POSIX, best-effort-atomic on Windows
        }
        finally
        {
            if (File.Exists(temp))
    ...
    private static string SanitizeFileName(string applicationId)
    {
        ...
        Span<char> sanitized = applicationId.Length <= 256 ? stackalloc char[applicationId.Length] : new char[applicationId.Length];
